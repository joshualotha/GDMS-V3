...
@section('header', 'GRN: ' . $goodsReceived->grn_number)
...
<div class="bg-white rounded-lg shadow p-6 mb-6">
    <div class="grid grid-cols-2 gap-6">
        <div>
            ...
            <p class="font-medium">{{ $goodsReceived->supplier->name }}</p>
        </div>
        <div>
            <p class="text-sm text-gray-500">Status</p>
            <p>
                @if($goodsReceived->status == 'pending')
                    <span class="px-2 py-1 text-xs rounded bg-yellow-100 text-yellow-800">Pending</span>
                @elseif($goodsReceived->status == 'completed')
        ...
        <div>
            ...
            <p class="font-medium">{{ $goodsReceived->purchaseOrder->po_number ?? 'N/A' }}</p>
        </div>
        <div>
    ...
    <table class="min-w-full divide-y divide-gray-200">
        ...
        <tfoot class="bg-gray-50">
            <tr>
                ...
                <td class="px-6 py-3 text-right font-bold">{{ number_format($goodsReceived->total_cost, 2) }}</td>
            </tr>
        </tfoot>
    </table>
</div>
